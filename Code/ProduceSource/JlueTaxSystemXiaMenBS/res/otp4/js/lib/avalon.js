    ...
    if (method) {
        ...
        switch (method) {
            case "add": //在pos位置后添加el数组（pos为插入位置,el为要插入的个数）
                 ...
                 * dingxr: begin
                 */
                //xf.info("--------fragments.length="+fragments.length);
                for(var n=0;n<fragments.length;n++){
	                var childVM=fragments[n].vmodels[0];
	                otp4.DiFactory.ready3(fragments[n].nodes,childVM);
                }
                /*
                xf.info("-----------------avalon.transation:"
                		+",nodeName="+$(transation)[0].nodeName
                		//+",txt="+$(transation)[0].textContent
                		+",childNodes.len="+$(transation)[0].childNodes.length
                		);
                if(transation.childNodes.length>0)
                	xf.info("node0="+$(transation.childNodes[1]).html());
                for(var n in transation){
                	if(typeof transation[n]!='function')
                		xf.info("-----------------avalon.transation:"+n);
                }
                */
                //dingxr end;
